...
| Motion | `patterns/motion.md` (83-sample re-synthesis) | is there a duration scale, and is `prefers-reduced-motion` handled (nine layers across 17 systems, up to an opt-in inversion that makes no-motion the default). **`scroll-behavior: smooth` and `scrollIntoView` are motion too** — easy to miss in a product with few transitions |
