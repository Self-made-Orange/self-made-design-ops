...
| freshness | **185** checked · 1 outdated (an Orbit prerelease, intentional) · 0 failed · 21 manual — **3 drifts handled (2026-08-18)** |
...
- [ ] **Backpack's `url` is effectively a dead address** — `backpack.github.io` returns 200
  at the root only and 404s everywhere below it; the real documentation is at
  `skyscanner.design/latest` (Supernova). The value is not `unverified`, so the automatic
  classification does not catch it — **a decision is needed on replacing it**
...
- [ ] `profiles/interpreted/` is **empty** — it gets filled when real product work needs an
  interpretation (building it in advance would only pile up unverified A values)
...
- [ ] **Turning Pages on in the repository settings (owner only)** —
  Settings → Pages → Build and deployment → Source: **GitHub Actions**.
  Once on, `.github/workflows/pages.yml` handles every deployment thereafter.
  **All three automation routes are blocked** (confirmed 2026-08-20):
...
- [ ] `tokens_format` values not registered in `TOKEN_LABEL_EN` pass through verbatim. No
  Korean value currently reaches the data, but a new sample harvested in Korean would, and
...
- [x] **Fixed the bug where the background dots were invisible** — `body::before` with
  ...
  draw
- [ ] **Looking at the live page remains the owner's job** — `keepyaoung.github.io` is blocked
  by this container's egress proxy, so the deployed result cannot be opened directly. What has
  been verified is that the deployment pipeline succeeds and that the files being deployed
  render correctly under headless
...
- [ ] **Satoshi has not been render-verified with the real typeface** — Fontshare is blocked
  by this container's proxy and Satoshi is not on npm. See the note in `site/README.md`;
  looking at the live page is the owner's check.
