...
| 축 | 상태 |
|----|------|
| 코퍼스 | **117개** (113 공개 + 문서층 2 + 코드층 신규 + 2026-09-03 추가 Geist — 구성: `design-systems/index.md` 상단) + 내부 표본 1 |
...
**잔여 (다음 사람이 이어받을 지점):**
- [ ] `a11y_target` 미확인 **57건** · `figma_kit` **49건** — 남은 최대 덩어리.
  상당수는 이미 C(선언 없음)일 가능성이 높지만 **확인 전까지는 미확인**입니다
  - ✅ **저장소 채널 소진 (2026-08-23)** — github `repo`를 가진 44건 전부를 WCAG 레벨 표기
    기준으로 전수 조사했습니다 (`main` 다음 `master`에서 README · docs/README ·
    ACCESSIBILITY.md). **적중 1건**: Codex를 README(`main@5a4ff8980f`)에서 WCAG 2.1 AA로
    해소했습니다. 39개는 아무 말이 없고, 3개는 읽을 수 있는 README가 없었으며, 57건 중
    14건은 github 저장소 자체가 없습니다. `UNVERIFIED.md`에 기록했습니다.
    **이걸로 나머지 43건이 C가 되지는 않습니다** — 목표를 문서 사이트에 적고 README에는
    안 적는 것이 평범한 경우이고, `figma_kit` 과대 계상이 방향만 반대인 같은 실수였습니다.
    **남은 경로는 문서 사이트 채널(B-1)이며 시스템별로 가야 합니다** — 다음 패스의 비용은
    그것입니다
- [x] ~~Odyssey~~ — **npm 경로로 해소 (2026-08-18)**: 토큰 전수 206
  ...
  SSO 뒤로 남음 — npm으로는 종결
- [x] ~~**Backpack `url`이 사실상 죽은 주소**~~ — **교체 완료 (2026-08-23).**
  이번 세션에서 실제로 재확인: `backpack.github.io/` **200**, `backpack.github.io/components/`
  **404**, `www.skyscanner.design/latest` **200** — 2026-08-18 관찰이 그대로 재현됩니다.
  `backpack.md`·`backpack.ko.md` 양쪽의 `url:`을 Supernova 주소로 바꾸고 참고 절에 교체 사실을
  기록했습니다. `verified:`는 그대로 뒀습니다(토큰 수확 시점 필드, 재측정한 토큰 없음).
  `systems.json`·`corpus.json` 재생성 완료.
  **일반화한 교훈: 틀린 값은 `미확인` 값보다 나쁩니다** — 자동 분류는 `미확인`만 감시하므로
  그냥 틀린 값에는 감시자가 아예 없습니다. 코퍼스에서 알려진 이런 사례는 이 건이 유일했습니다
...
- [ ] **월간 freshness CI** 결과 처리 (절차: `freshness.md` — 실례 4건:
  ...
  - 🔻 **낡음 24건 선별 (2026-08-23) — 실작업은 10건.** 구성은
    ...
    **기록 값을 하나도 바꾸지 않았습니다.** 값이 안 움직이는 승급은 `source:` 문자열
    수정 비용만 들고 얻는 게 없습니다. **minor 10건**만 "우리가 기록한 절" 단위로
    재수확하고, patch는 major가 뜨거나 다른 이유로 그 항목을 열 때 함께 봅니다.
    **Orbit 행은 드리프트가 아닙니다** — 업스트림 `@kiwicom/orbit-components`가
    `latest` dist-tag를 `27.7.1-alpha.0` 프리릴리스에 걸어둔 상태라 검사기가 그걸
    최신으로 읽습니다. 이미 "의도"로 판정된 건이므로 재수확이 아니라 **보고서의 예외
    표기** 대상입니다
...
- [x] ~~`profiles/interpreted/` **비어 있음**~~ — **기술이 낡았습니다 (2026-08-23 정정).**
  2026-08-19부터 `self-made-designops-site.DESIGN.md`(+ 한국어 판본)가 들어 있고,
  `site/design-spec.mjs`가 `docs/assets/site.css`에서 생성합니다. 이 코퍼스가 처음 만든
  제품이 사이트였으므로 해석이 처음 필요해진 것도 사이트였습니다 — 이 항목이 적어둔 조건
  그대로입니다. 원칙은 그대로: **실제 제품 작업이 필요로 할 때만** 늘리고 미리 만들지 않습니다
...
- [x] ~~**저장소 설정에서 Pages 켜기 (소유자만 가능)**~~ — **완료, 사이트가 살아 있습니다
  (2026-08-23 확인).** `https://keepyaoung.github.io/self-made-design-ops/`가 **200**과 올바른
  `<title>`을 반환하고, 거기서 서빙되는 `data/corpus.json`이 **116개 시스템**으로 로컬 빌드와
  일치합니다. `pages.yml`이 설계대로 배포를 맡고 있습니다.
  자동화가 왜 불가능했는지는 기록으로 아래에 남깁니다:
...
- [x] ~~사이트 문안 언어~~ — **영어로 전환 (2026-08-19).** 코퍼스 이주가 끝난
  2026-08-21부터는 링크 대상 문서가 한국어라고 말하지 않고, 영어 주노출 + 한국어 원문 병기로
  적습니다. 데이터에서 오는 한국어 값은 `TOKEN_LABEL_EN` 표에 있는 것만 영어 라벨을 붙이고
  원문을 툴팁으로 남기지만, **2026-08-21 기준 `corpus.json`에 도달하는 한국어 값은 0건**이라
  이 표는 실제로 쓰이지 않는 폴백입니다 (2026-08-23, 영문판과 어긋나 있던 것을 정정)
- [x] ~~`TOKEN_LABEL_EN` 미등록 `tokens_format` 값은 한국어 원문 그대로 나갑니다~~ —
  **태스크에서 내림 (2026-08-23).** 2026-08-21에 번역 이주가 끝나 **`corpus.json`에 도달하는
  한국어 값이 0건**입니다. 표는 지금 아무것도 통과시키지 않는 폴백입니다. 일어나지 않는 조건에
  검사기를 붙이는 것은 가정에 대고 하는 작업입니다. 실제로 한국어로 수확한 표본이 들어오는 날
  다시 엽니다 — 그때가 라벨과 검사 둘 다 싸고 근거가 있는 시점입니다
- [x] ~~`docs/index.html`의 축 한 줄 결론 9개에 자동 검증이 없습니다~~ —
  **`site/check-headlines.mjs`를 만들어 CI에 연결했습니다 (2026-08-23).** 헤드라인이 말하는
  숫자를 원본 문서의 표에서 다시 세고, 인용한 값이 아직 본문에 있는지 확인합니다.
  `--strict`가 PR을 실패시킵니다. 산문을 판정하지는 않습니다 — 기계로 확인할 것이 없는
  주장은 통과가 아니라 **미검사**로 보고하며, 지금은 `table.md`·`navigation.md`가 여기
  해당합니다(결론이 그 축이 무엇을 다루는지만 말합니다).
  **만드는 과정에서 잡으려던 바로 그 드리프트를 찾았습니다** — `motion.md`의 reduced-motion
  표가 **9개 층 · 17개 시스템**으로 늘었는데 같은 문서의 세 문장이 여전히
  "6개 층 · 최소 13개 시스템"이라고 말하고 있었고, 사이트 헤드라인 ·
  `agents/design-review.md` · FRR 사례연구가 전부 낡은 숫자를 인용하고 있었습니다.
  전부 정정했습니다. **이 실패의 세 번째 사례**입니다 (typography 13 → 17,
  `scales.md` 6 → 8에 이어)

## G. 사이트 시각 정리 (2026-08-20)

- [x] ~~**다크 온리 전환**~~ — **이 항목은 더 이상 사이트를 기술하지 않습니다
  (2026-08-23 정정).** 제거했다고 적힌 셋이 전부 `docs/assets/site.css`·`docs/index.html`에
  살아 있습니다 — `color-scheme: light`인 라이트 `:root`, `[data-theme="dark"]` 블록,
  토글 버튼, 첫 페인트 전 FOUC 스크립트. 액센트도 `#1f6feb`이 아니라 주황입니다
  (칠은 `--accent:#f97316`, 버튼은 `--btn:#ff5926`). 2026-08-23에 두 테마 모두 렌더해
  확인했습니다. 다크 온리를 되돌린 작업은 여기 기록되지 않았습니다 —
  **정본은 이 줄이 아니라 스타일시트입니다**
...
- [x] ~~**라이브 페이지 육안 확인은 소유자 몫으로 남음**~~ — **차단이 풀렸습니다
  (2026-08-23).** 이번 세션에서 `keepyaoung.github.io`에 접근됐습니다: 페이지가 **200**과
  예상한 `<title>`을 반환하고, 거기서 서빙되는 `data/corpus.json`이 로컬 빌드와 같은
  **116개 시스템**을 보고합니다 — 배포된 산출물이 소스와 일치합니다. 여전히 소유자 몫인 것은
  **시각적** 판단입니다. HTTP와 JSON은 올바른 바이트가 도착했다고 말할 뿐 페이지가 보기 좋다고
  말하지 않습니다. 위의 이그레스 차단 서술은 2026-08-20 세션의 사실이지 모든 세션의 항구적
  성질이 아닙니다 — **물려받지 말고 매번 다시 확인할 것**

## H. 번역 패스에서 나온 미해결 (2026-08-21 신설)
