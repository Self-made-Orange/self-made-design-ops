...
| | 8/18 이전 | 1차 분류 후 | B-1 렌더 후 | **2026-08-23 재집계** |
|---|:---:|:---:|:---:|:---:|
| 열린 미확인 표기 | 428 | 359 | 308 | **212** |
| frontmatter 미확인 필드 | — | — | 118 | **109** |
| — `a11y_target` | — | — | 59 | **57** |
| — `figma_kit` | — | — | 49 | **48** |
| — `license` | — | — | 9 | **4** |
| — `repo` | — | — | 1 | **0** |

> **308은 애초에 재현되지 않는 숫자였고, 원인은 재집계 절차입니다.** 이 문서가 제시하던
> 명령에 결함이 둘 있었고 아래 「재점검 절차」에서 고쳤습니다. 하나는 `systems/*.md`로
> 글롭해서 `.ko.md` 쌍까지 쓸어 담아 모든 항목을 두 번 세는 것 — **2026-08-21에
> `check-sources.mjs`에서 잡은 것과 같은 버그**로, 거기서는 소스 185개가 370개로 보고됐습니다.
> 다른 하나는 `unverified`를 대소문자 구분해 매칭해서 영어 정본의 문장 첫머리
> **`Unverified`**를 전부 놓치는 것입니다. 기재된 명령은 오늘도 367을 반환하고, 고친 명령은
> **212**를 반환합니다. `license`·`repo` 수치는 해당 항목이 해소된 뒤로 재집계된 적이
> 없었습니다.
...
## `figma_kit`도 같은 방식으로 전수 조사 (2026-08-23)

`figma_kit: 미확인` 중 github 저장소를 가진 **41건**에 같은 스윕을 돌렸습니다.
`README.md` · `docs/README.md` · `CONTRIBUTING.md` · `docs/index.md` · `.github/README.md`를
읽고, 매칭 조건은 일부러 좁게 잡았습니다 — **`figma.com/file` · `figma.com/design` ·
`figma.com/community/file` URL**만 인정하고 "Figma"라는 단어 언급은 세지 않았습니다.
"우리는 Figma로 디자인합니다"는 킷이 아닙니다.

| 결과 | 건수 |
|------|:---:|
| **Figma 파일을 링크함** | **1** |
| 저장소에 그런 링크 없음 | 35 |
| 시도한 경로에 읽을 수 있는 README 없음 | 4 (Bolt · Nord · Odyssey · Origami) |
| 49건 중 github 저장소 자체가 없음 | 8 |

**Kaizen이고, 이게 흥미로운 사례입니다.** 2026-08-18 문서 사이트 렌더는 킷이
*시사되지만 공개되지 않았다*고 결론 내고 그 근거로 미확인을 유지했습니다. 그런데
`CONTRIBUTING.md` 165행이 그것을 그냥 링크합니다 —
`figma.com/file/eZKEE5kXbEMY3lx84oz8iN`, 파일 이름 "💜 UI Kit: Heart". 문서 사이트 렌더가
결코 닿지 않는 파일입니다. Backpack과 같은 판독으로 `true`로 해소했습니다. 주소가 302를
반환하므로 확인되는 것은 **킷의 존재**이지 열람 가능 여부가 아닙니다. 파일 키를 항목에
기록했습니다. 다시 식별할 수 없는 킷은 재확인도 할 수 없기 때문입니다.

**교훈은 Kaizen이 아니라 채널에 관한 것입니다.** 문서 사이트 렌더와 저장소 열람은 서로
다른 채널인데, 이 코퍼스는 앞의 것이 뒤의 것을 포함한다고 다뤄 왔습니다. 사이트 렌더 후에도
값이 미확인으로 남아 있다면, 차단으로 분류하기 전에 저장소를 한 번 보는 것이 값어치가
있습니다.
...
1. **현황 재계산**
   ...
   # 영어 정본만 — .ko.md 쌍을 함께 세면 전부 두 번 계상됩니다. 그리고 대소문자를
   # 무시해야 문장 첫머리의 "Unverified"를 놓치지 않습니다.
   FILES=$(ls *.md | grep -v '\.ko\.md')
   echo "$FILES" | tr '\n' '\0' | xargs -0 grep -ohiE "unverified|미확인" | wc -l          # 총 등장
   echo "$FILES" | tr '\n' '\0' | xargs -0 grep -ohiE "~~[^~]*(unverified|미확인)[^~]*~~" | wc -l   # 취소선(기해소)
   ```
   차이가 **열린 미확인**입니다. 이 문서의 「총계」 표를 갱신하세요.
   frontmatter 수치는 대신 생성 데이터에서 뽑습니다 — 이쪽은 낡을 수 없습니다:
   ```bash
